(function(){
  'use strict';
  if(window.__tasneefCoreSalaryTabV438) return;
  window.__tasneefCoreSalaryTabV438=true;
  ...
  function ensureTab(){
    ...
    if(!btn.__salaryStableV438){
      btn.__salaryStableV438=true;
  ...
  function mountSalarySection(){
    ...
    if(!window.tasneefSalariesUnifiedV438){
      mount.innerHTML='<div class="cu413-msg err">ملف الرواتب لم يتم تحميله بعد. ارفع tasneef_salaries_unified_source_v438.js ثم حدّث الكاش.</div>';
      return false;
    }
    ...
    try{ window.tasneefSalariesUnifiedV438.inject(); }catch(e){ mount.innerHTML='<div class="cu413-msg err">تعذر إنشاء الرواتب: '+(e.message||e)+'</div>'; return false; }
    const sal=$('salaries');
  ...
  function showSalary(){
    ...
    try{
      ...
      if(mountSalarySection()){
        setTimeout(()=>{ try{ window.tasneefSalariesUnifiedV438?.load?.(); }catch(e){ msg('تعذر تحديث الرواتب: '+(e.message||e),true); } },60);
  ...
  window.tasneefCoreSalaryTabV438={show:showSalary,patch};
})();
