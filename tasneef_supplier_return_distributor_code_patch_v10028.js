/* Tasneef Supplier Return + Distributor Code Patch v10028
...
(function(){
  'use strict';
  if(window.__tasneefSupplierReturnDistributorPatchV10028Started) return;
  window.__tasneefSupplierReturnDistributorPatchV10028Started = true;

  const VERSION = 'supplier_return_distributor_code_v10028';
  ...
  function ensureMiniStyle(){
    if($('supplierReturnDistributorPatchStyleV10028')) return;
    const st=document.createElement('style');
    st.id='supplierReturnDistributorPatchStyleV10028';
    st.textContent = `
      .tasneef-code-match-v10028{margin-top:6px;background:#edf8f3;border:1px solid #cfe8dd;color:#07513f;border-radius:11px;padding:7px 9px;font-size:12px;font-weight:800}
      .tasneef-code-match-v10028.bad{background:#fff4df;border-color:#efd19a;color:#7a5200}
      .tasneef-return-modal-v10028 .grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:10px}.tasneef-return-modal-v10028 label{display:block;margin:8px 0 4px;font-weight:800;color:#073d31}.tasneef-return-modal-v10028 input,.tasneef-return-modal-v10028 select,.tasneef-return-modal-v10028 textarea{width:100%}
      .finance-print-actions-v10028{display:flex;gap:8px;flex-wrap:wrap;margin:10px 0}.finance-print-actions-v10028 button{border-radius:12px;padding:9px 13px}
      @media(max-width:700px){.tasneef-return-modal-v10028 .grid{grid-template-columns:1fr}}
      @media print{.no-print-v10028,.fin-actions button,.finance-print-actions-v10028{display:none!important}.modal-backdrop{position:static!important;background:#fff!important;display:block!important;padding:0!important}.card{box-shadow:none!important}}
    `;
  ...
  function showCodeMatch(item, code){
    let box=$('finDistributorCodeMatchV10028');
    const input=$('finLineDistributorCodeV15');
    if(!input) return;
    if(!box){
      box=document.createElement('div');
      box.id='finDistributorCodeMatchV10028';
      input.insertAdjacentElement('afterend', box);
    }
    if(item){
      box.className='tasneef-code-match-v10028';
      box.innerHTML='تم التعرف على المنتج: <b>'+esc(item.name||'-')+'</b> — الكود الداخلي: <b>'+esc(itemCode(item)||'-')+'</b>';
    }else if(S(code)){
      box.className='tasneef-code-match-v10028 bad';
  ...
  function attachDistributorCodeRecognition(){
    ensureMiniStyle();
    const input=$('finLineDistributorCodeV15');
    if(!input || input.__tasneefDistributorRecognitionV10028) return;
    input.__tasneefDistributorRecognitionV10028 = true;
  ...
  window.financeProOpenSupplierReturnV10028 = function(supplier){
    supplier=S(supplier);
    if(!supplier) return alert('اسم المورد غير واضح');
    ensureMiniStyle();
    const html=`
      <div class="modal-backdrop tasneef-return-modal-v10028" onclick="if(event.target===this)this.remove()" style="position:fixed;inset:0;background:rgba(0,35,28,.45);z-index:999999;display:grid;place-items:center;padding:18px">
        <div class="card" style="width:min(760px,96vw);max-height:92vh;overflow:auto">
          ...
          <div class="grid">
            <div><label>المنتج</label><select id="supplierReturnItemV10028">${supplierOptionsForReturn(supplier)}</select></div>
            <div><label>الكمية المرتجعة</label><input id="supplierReturnQtyV10028" type="number" min="0" step="0.01" placeholder="مثال 2"></div>
            <div><label>التاريخ</label><input id="supplierReturnDateV10028" type="date" value="${today()}"></div>
            <div><label>رقم فاتورة المورد</label><input id="supplierReturnInvoiceV10028" placeholder="اختياري"></div>
          </div>
          <label>ملاحظة</label><textarea id="supplierReturnNoteV10028" placeholder="سبب الإرجاع أو أي ملاحظة"></textarea>
          ...
          <div class="fin-actions" style="margin-top:12px"><button onclick="financeProSaveSupplierReturnV10028('${esc(supplier).replace(/'/g,'&#039;')}', this)">حفظ المرتجع</button></div>
        </div>
      </div>`;
  ...
  async function recalcOrFallback(item, qty){
    ...
    try{
      const rpc = await sb.rpc('tasneef_recalc_inventory_item_qty_v10028', {p_item_id:String(item.id)});
      if(!rpc.error) return;
    }catch(_){ }
    try{
  ...
  window.financeProSaveSupplierReturnV10028 = async function(supplier, btn){
    try{
      if(btn) btn.disabled=true;
      if(!window.sb) throw new Error('الاتصال غير جاهز');
      supplier=S(supplier);
      const itemId=S($('supplierReturnItemV10028')?.value);
      const item=A(state().items).find(i=>String(i.id)===itemId);
      if(!item) throw new Error('اختر المنتج');
      const qty=N($('supplierReturnQtyV10028')?.value);
      ...
      const date=S($('supplierReturnDateV10028')?.value)||today();
      const supplierInvoiceNo=S($('supplierReturnInvoiceV10028')?.value);
      const note=S($('supplierReturnNoteV10028')?.value);
      ...
      document.querySelector('.tasneef-return-modal-v10028')?.remove();
  ...
  function patchSupplierModalButtons(){
    document.querySelectorAll('.modal-backdrop .card').forEach(card=>{
      ...
      if(actions && !card.querySelector('[data-supplier-return-v10028]')){
        const b=document.createElement('button');
        b.type='button';
        b.className='light';
        b.dataset.supplierReturnV10028='1';
        b.textContent='تسجيل مرتجع للمورد';
        b.onclick=function(){ window.financeProOpenSupplierReturnV10028(supplier); };
  ...
  window.financePrintCurrentV10028 = function(){
    const box=$('financeDashboard');
    if(!box) return alert('قسم المالية غير مفتوح');
    const clone=box.cloneNode(true);
    clone.querySelectorAll('button,input,select,textarea,.finance-print-actions-v10028,.no-print-v10028').forEach(x=>x.remove());
    printHtml('طباعة قسم المالية والمخزون', clone.innerHTML);
  };
  window.financePrintVisibleReportV10028 = function(){
  ...
  window.financePrintModalV10028 = function(btn){
    const card=btn && btn.closest ? btn.closest('.card') : document.querySelector('.modal-backdrop .card');
    if(!card) return;
    const clone=card.cloneNode(true);
    clone.querySelectorAll('button,.fin-tabs,.no-print-v10028').forEach(x=>x.remove());
  ...
  function addFinancePrintButtons(){
    ...
    if(!body || dash.querySelector('#financePrintActionsV10028')) return;
    const div=document.createElement('div');
    div.id='financePrintActionsV10028';
    div.className='finance-print-actions-v10028 no-print-v10028';
    div.innerHTML='<button type="button" class="light" onclick="financePrintCurrentV10028()">طباعة قسم المالية</button><button type="button" class="light" onclick="financePrintVisibleReportV10028()">طباعة التقرير الحالي</button>';
  ...
  function showProductDetailsFixed(id){
    ...
    const outs=moves.filter(m=>S(m.movement_type)==='out');
    ...
    const calculatedQty=Math.max(0, inQty + returnQty - outQty - consumedQty - damagedQty - supplierReturnQty);
    const hasDbQty = item && item.quantity !== undefined && item.quantity !== null && S(item.quantity) !== '';
    const currentQty = hasDbQty ? N(item.quantity) : calculatedQty;
    ...
    const modal=`<div class="modal-backdrop" onclick="if(event.target===this)this.remove()" style="position:fixed;inset:0;background:rgba(0,35,28,.45);z-index:999999;display:grid;place-items:center;padding:18px">
      <div class="card" style="width:min(1120px,96vw);max-height:92vh;overflow:auto">
        <div class="fin-actions no-print-v10028" style="justify-content:space-between"><h2>بيانات المنتج: ${esc(item.name||'-')}</h2><div><button class="light" onclick="financePrintModalV10028(this)">طباعة</button><button class="danger" onclick="this.closest('.modal-backdrop').remove()">إغلاق</button></div></div>
        <div class="fin-tabs no-print-v10028" style="grid-template-columns:repeat(7,1fr);margin-bottom:12px">
        ...
        <section class="v10028-page" data-page="0"><div class="fin-grid">${img?`<div class="fin-card">${img}</div>`:''}<div class="fin-card fin-kpi"><small>الداخل</small><b>${N(inQty)}</b></div><div class="fin-card fin-kpi"><small>الخارج</small><b>${N(outQty)}</b></div><div class="fin-card fin-kpi"><small>المستهلك</small><b>${N(consumedQty)}</b></div><div class="fin-card fin-kpi"><small>المرتجع للمخزون</small><b>${N(returnQty)}</b></div><div class="fin-card fin-kpi"><small>مرتجع للمورد</small><b>${N(supplierReturnQty)}</b></div><div class="fin-card fin-kpi"><small>تالف / هدر / سكراب</small><b>${N(damagedQty)}</b></div><div class="fin-card fin-kpi"><small>الرصيد الحالي</small><b>${N(currentQty)}</b></div></div><div class="fin-soft">الكود: <b>${esc(itemCode(item)||'-')}</b> | كود الموزع: <b>${esc(itemSupplierCode(item)||'-')}</b> | الوحدة: <b>${esc(item.unit||'-')}</b></div><div class="fin-soft" style="margin-top:8px">معادلة العرض: الداخل + المرتجع للمخزون - الخارج - المستهلك - التالف/الهدر/السكراب - مرتجع المورد = <b>${N(calculatedQty)}</b></div></section>
        <section class="v10028-page hidden" data-page="1"><div class="fin-table"><table><thead><tr><th>التاريخ</th><th>النوع</th><th>الكمية</th><th>المورد</th><th>البيان</th><th>القيمة</th></tr></thead><tbody>${rowsHtml(ins)}</tbody></table></div></section>
        <section class="v10028-page hidden" data-page="2"><div class="fin-table"><table><thead><tr><th>التاريخ</th><th>النوع</th><th>الكمية</th><th>المستلم</th><th>البيان</th><th>القيمة</th></tr></thead><tbody>${rowsHtml(outs)}</tbody></table></div></section>
        <section class="v10028-page hidden" data-page="3"><div class="fin-table"><table><thead><tr><th>التاريخ</th><th>النوع</th><th>الكمية</th><th>المستلم</th><th>البيان</th><th>القيمة</th></tr></thead><tbody>${rowsHtml(consumed)}</tbody></table></div></section>
        <section class="v10028-page hidden" data-page="4"><div class="fin-table"><table><thead><tr><th>التاريخ</th><th>النوع</th><th>الكمية</th><th>المستلم</th><th>البيان</th><th>القيمة</th></tr></thead><tbody>${rowsHtml(returns)}</tbody></table></div></section>
        <section class="v10028-page hidden" data-page="5"><div class="fin-table"><table><thead><tr><th>التاريخ</th><th>النوع</th><th>الكمية</th><th>المورد</th><th>البيان</th><th>القيمة</th></tr></thead><tbody>${rowsHtml(supplierReturns)}</tbody></table></div></section>
        <section class="v10028-page hidden" data-page="6"><div class="fin-table"><table><thead><tr><th>التاريخ</th><th>النوع</th><th>الكمية</th><th>المستلم</th><th>البيان</th><th>القيمة</th></tr></thead><tbody>${rowsHtml(damaged)}</tbody></table></div></section>
      </div>
    </div>`;
    document.body.insertAdjacentHTML('beforeend', modal);
  }
  showProductDetailsFixed.__supplierReturnV10028 = true;
  ...
  function patchButtonsDirectly(){
    document.querySelectorAll('[onclick^="financeProShowProductV15"]').forEach(btn=>{
      ...
      if(!m || btn.__showProductV10028) return;
      btn.__showProductV10028=true;
