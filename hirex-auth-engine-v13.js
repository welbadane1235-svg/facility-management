/* HireX Auth Engine V13 - clean auth layer, does not block browse buttons. */
(function(){
  'use strict';
  var sbClient = null;
  ...
  function notify(msg){
    ...
    var old = qs('#hxAuthToastV13'); if(old) old.remove();
    var el = document.createElement('div');
    el.id = 'hxAuthToastV13';
    el.textContent = msg;
  ...
  function ensureModal(){
    if(qs('#hxAuthModalV13')) return;
    var style = document.createElement('style');
    style.id = 'hxAuthStyleV13';
    style.textContent = '#hxAuthModalV13{position:fixed;inset:0;display:none;align-items:center;justify-content:center;z-index:2147483646;background:rgba(0,0,0,.74);backdrop-filter:blur(10px);direction:rtl}#hxAuthModalV13.show{display:flex}.hx13-card{width:min(430px,92vw);box-sizing:border-box;background:linear-gradient(180deg,#07302d,#020908);border:1px solid rgba(0,245,245,.38);box-shadow:0 28px 90px rgba(0,0,0,.65);border-radius:24px;padding:20px;color:#eaffff;font-family:Arial,Tahoma,sans-serif}.hx13-head{display:flex;justify-content:space-between;align-items:center;gap:10px;margin-bottom:10px}.hx13-title{font-size:22px;font-weight:900}.hx13-close{width:40px;height:40px;border-radius:13px;border:1px solid rgba(255,255,255,.16);background:rgba(255,255,255,.07);color:#fff;font-size:22px;cursor:pointer}.hx13-reason{font-size:13px;font-weight:800;line-height:1.7;background:rgba(0,245,245,.08);border:1px solid rgba(0,245,245,.22);padding:10px;border-radius:14px;margin:10px 0;text-align:center}.hx13-tabs{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:12px 0}.hx13-tabs button{border:1px solid rgba(0,245,245,.25);background:rgba(255,255,255,.05);color:#dfffff;border-radius:13px;padding:11px;font-weight:900;cursor:pointer}.hx13-tabs button.active{background:#16d6d6;color:#001313}.hx13-field{margin:10px 0}.hx13-field label{display:block;font-size:12px;color:#bff;font-weight:900;margin-bottom:6px}.hx13-field input{width:100%;box-sizing:border-box;border:1px solid rgba(0,245,245,.28);background:#071716;color:#fff;border-radius:13px;padding:13px 14px;font-weight:800;outline:none}.hx13-submit{width:100%;border:0;border-radius:14px;background:#16d6d6;color:#001313;padding:14px;margin-top:10px;font-weight:900;cursor:pointer}.hx13-link{width:100%;border:1px solid rgba(255,255,255,.16);border-radius:12px;background:transparent;color:#eaffff;padding:11px;margin-top:8px;font-weight:800;cursor:pointer}.hx13-msg{min-height:24px;color:#ffd98a;text-align:center;font-size:12px;line-height:1.7;margin-top:10px}';
    document.head.appendChild(style);
    var modal = document.createElement('div');
    modal.id = 'hxAuthModalV13';
    modal.innerHTML = '<div class="hx13-card" role="dialog" aria-modal="true"><div class="hx13-head"><div class="hx13-title" id="hx13Title">تسجيل الدخول</div><button type="button" class="hx13-close" id="hx13Close">×</button></div><div class="hx13-reason" id="hx13Reason">سجل دخولك لإكمال هذا الإجراء.</div><div class="hx13-tabs"><button type="button" data-hx13-mode="login" class="active">دخول</button><button type="button" data-hx13-mode="signup">إنشاء حساب</button></div><div class="hx13-field" id="hx13NameWrap" style="display:none"><label>الاسم الكامل</label><input id="hx13Name" autocomplete="name" placeholder="اكتب اسمك"></div><div class="hx13-field"><label>البريد الإلكتروني</label><input id="hx13Email" type="email" autocomplete="email" placeholder="example@email.com"></div><div class="hx13-field"><label>كلمة المرور</label><input id="hx13Password" type="password" autocomplete="current-password" placeholder="******"></div><button type="button" class="hx13-submit" id="hx13Submit">تسجيل الدخول</button><button type="button" class="hx13-link" id="hx13Resend">إعادة إرسال رابط التحقق</button><button type="button" class="hx13-link" id="hx13Reset">نسيت كلمة المرور؟</button><div class="hx13-msg" id="hx13Msg"></div></div>';
    document.body.appendChild(modal);
    modal.addEventListener('click', function(e){ if(e.target === modal) closeModal(); });
    qs('#hx13Close').addEventListener('click', closeModal);
    qsa('[data-hx13-mode]').forEach(function(btn){ btn.addEventListener('click', function(){ setMode(btn.getAttribute('data-hx13-mode')); }); });
    qs('#hx13Submit').addEventListener('click', submitAuth);
    qs('#hx13Resend').addEventListener('click', resendVerify);
    qs('#hx13Reset').addEventListener('click', resetPassword);
    qs('#hx13Password').addEventListener('keydown', function(e){ if(e.key === 'Enter') submitAuth(); });
  }
  function message(txt){ var el = qs('#hx13Msg'); if(el) el.textContent = txt || ''; }
  function setMode(mode){
    ensureModal();
    var signup = mode === 'signup';
    qs('#hxAuthModalV13').setAttribute('data-mode', signup ? 'signup' : 'login');
    qsa('[data-hx13-mode]').forEach(function(btn){ btn.classList.toggle('active', btn.getAttribute('data-hx13-mode') === (signup ? 'signup':'login')); });
    qs('#hx13NameWrap').style.display = signup ? 'block' : 'none';
    qs('#hx13Title').textContent = signup ? 'إنشاء حساب وتأكيد البريد' : 'تسجيل الدخول';
    qs('#hx13Submit').textContent = signup ? 'إنشاء حساب وإرسال التحقق' : 'تسجيل الدخول';
    message(signup ? 'سيتم إرسال رابط تحقق إلى بريدك الإلكتروني.' : 'استخدم بريدك وكلمة المرور للدخول.');
  }
  function openModal(reason, mode){
    ensureModal();
    qs('#hx13Reason').textContent = reason || 'سجل دخولك لإكمال هذا الإجراء.';
    setMode(mode || 'login');
    qs('#hxAuthModalV13').classList.add('show');
    document.body.classList.add('modal-open');
    setTimeout(function(){ var el = qs('#hx13Email'); if(el) el.focus(); }, 80);
  }
  function closeModal(){ var m=qs('#hxAuthModalV13'); if(m) m.classList.remove('show'); document.body.classList.remove('modal-open'); }
  ...
  async function submitAuth(){
    ...
    var mode = (qs('#hxAuthModalV13') && qs('#hxAuthModalV13').getAttribute('data-mode')) || 'login';
    var email = (qs('#hx13Email').value || '').trim().toLowerCase();
    var password = qs('#hx13Password').value || '';
    var fullName = (qs('#hx13Name').value || '').trim();
  ...
  function installLogoutPatch(){
    ...
    if(typeof logout !== 'function' || logout.__hx13) return;
    ...
    window.accountLogout.__hx13 = true;
  }
  function init(){
    ensureModal();
  ...
  window.hirexAuthV13 = { version:'13', require: requireAuth, open: function(reason){ openModal(reason || 'سجل دخولك لإكمال هذا الإجراء.', 'login'); }, signup: function(reason){ openModal(reason || 'أنشئ حسابك لإكمال هذا الإجراء.', 'signup'); }, isVerified: isVerified };
