(function(){
  'use strict';
  const BUILD='V429 نفس بيانات الحضور القديم - حفظ آمن وجلب السجلات';
  ...
  function attAliases(a){
    ...
    add('code',a.worker_employee_code||a.worker_code||a.employee_code);
    add('name',a.worker_identity||a.worker_name||a.name);
    if(a.worker_id){
      add('code',workerCodeFromOldId(a.worker_id));
      add('name',workerNameFromOldId(a.worker_id));
      // لو كان النظام القديم يحفظ رقم العامل فقط، نحفظه كمعرف احتياطي ولا نجعله يطغى على الاسم.
      add('oldid',a.worker_id);
    }
    return [...new Set(out.filter(x=>!x.endsWith(':')))];
  }
  function rowAliases(r){
    const out=[]; const add=(prefix,val)=>{val=S(val); if(val) out.push(prefix+':'+norm(val));};
    add('code',distCode(r));
    add('name',distName(r));
    // بعض أسماء التوزيع تأتي بصيغة: TS-01 - الاسم، فنضيف الاسم بدون الكود للمطابقة مع الحضور القديم.
    const dn=distName(r).replace(/^TS-?\d+\s*[-–—:]\s*/i,'').trim();
    if(dn && dn!==distName(r)) add('name',dn);
    return [...new Set(out.filter(x=>!x.endsWith(':')))];
  }
  ...
  async function load(month,force){
    ...
    const [dr,ar,wr]=await Promise.all([
      safe('monthly_distribution', c.from('monthly_distribution').select('*').eq('month_key',month).limit(20000)),
      safe('attendance', c.from('attendance').select('*').gte('attendance_date',month+'-01').lt('attendance_date',nextMonth(month)+'-01').limit(20000)),
      safe('workers', c.from('workers').select('*').limit(20000))
    ]);
    state={dist:mergeDistribution(dr.data||[]),att:ar.data||[],workers:wr.data||[],month};
  }
  ...
  function render(){
    ...
    let p=0,a=0,o=0; rows.forEach(r=>{const st=statusCode(getRowRec(amap,r,f.date)?.status||'present'); if(st==='present')p++; else if(st==='absent')a++; else o++;});
  ...
  async function save(){
    try{const c=client(); if(!c)return msg('Supabase غير جاهز',true); const f=filters(); const rows=currentRows(); if(!rows.length)return msg('لا يوجد عمال للحفظ.',true);
      ...
      const payload=rows.map(r=>{const el=card.get(workerUniqKey(r)); if(!el)return null; return {worker_identity:distName(r),worker_name:distName(r),worker_identity:distName(r),worker_name:distName(r),worker_employee_code:distCode(r),worker_code:distCode(r),project_id:distProjectId(r)||((r.__projectIds||[])[0]||''),supervisor_id:numericId(distSupCode(r)),supervisor_employee_code:distSupCode(r),status:S(el.querySelector('select')?.value||'present'),notes:S(el.querySelector('input')?.value||'')};}).filter(Boolean);
  ...
  window.tasneefCoreAttendanceV429={refresh,save,activate};
  console.log('Tasneef core attendance V429 loaded');
})();
