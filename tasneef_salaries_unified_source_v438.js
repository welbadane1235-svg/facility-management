(function(){
  'use strict';
  if(window.__tasneefSalariesUnifiedV438) return;
  window.__tasneefSalariesUnifiedV438=true;
  const VERSION='V438 تصحيح مصدر الرواتب الموحد';
  ...
  const monthEnd=m=>{const [y,mo]=m.split('-').map(Number); const d=new Date(y,mo,0); return d.getFullYear()+'-'+String(d.getMonth()+1).padStart(2,'0')+'-'+String(d.getDate()).padStart(2,'0')};
  const norm=s=>S(s).toLowerCase().replace(/[إأآا]/g,'ا').replace(/[ىي]/g,'ي').replace(/ة/g,'ه').replace(/[ًٌٍَُِّْـ]/g,'').replace(/[\u200e\u200f]/g,'').replace(/\s+/g,' ').trim();
  ...
  function msg(t,bad){let el=$('salaryMsg')||$('cu413Msg'); if(el){el.textContent=t; el.className=(el.id==='salaryMsg'?'msg ':'cu413-msg ')+(bad?'err':''); el.classList?.remove('hidden'); el.style.display='block';} console[bad?'warn':'log']('[salary]',t);}
  ...
  async function tryAll(table,select='*',build){try{return await all(table,select,build)}catch(e){console.warn('salary skip '+table,e.message);return[]}}
  ...
  function allowanceOf(w){return N(w.allowance ?? w.allowances ?? w.salary_allowance ?? w.allowance_amount ?? w.housing_allowance ?? w.allowance_value ?? 0)}
  ...
  function projectName(pid){return S((st.projects||[]).find(p=>S(p.id)===S(pid))?.name||'')}

  function mergeWorkers(empRows,workerRows){
    const m=new Map();
    function put(w,priority){
      const c=codeOf(w); if(!c || /^\d+$/.test(c)) return; // الرواتب تعتمد كود TS من النظام الموحد
      const old=m.get(c)||{};
      const merged= priority>=(old.__priority||0) ? Object.assign({},old,w,{__priority:priority}) : Object.assign({},w,old,{__priority:old.__priority||0});
      m.set(c,merged);
    }
    (workerRows||[]).forEach(w=>put(w,1));
    (empRows||[]).forEach(w=>put(w,2)); // بيانات العمال الموحدة لها أولوية في الراتب والبدلات والإقامة
  ...
  function savedFor(row,month){const code=codeNorm(row.employee_ts_id||row.employee_code); const nid=String(numericEntityId(row,0)); return (st.saved||[]).find(s=>S(s.salary_month).slice(0,7)===month && (codeNorm(s.employee_ts_id||s.employee_code)===code || String(s.entity_id)===nid))||{};}

  function matchAttendance(a,row){
    const keys=[row.employee_ts_id,row.employee_code,row.employee_name,row.residency_name].map(x=>norm(codeNorm(x))).filter(Boolean);
  ...
  function attendanceStats(row,month){const p=new Set(), a=new Set(), allDates=new Set();
    (st.attendance||[]).forEach(r=>{const d=S(r.attendance_date||r.date||r.created_at).slice(0,10); if(!d.startsWith(month))return; if(!matchAttendance(r,row))return; allDates.add(d); if(isAbsent(r.status||r.state||r.attendance_status))a.add(d); else if(isPresent(r.status||r.state||r.attendance_status))p.add(d);});
    a.forEach(d=>p.delete(d)); return {present:p.size, absent:a.size, dates:[...allDates].sort(), absentDates:[...a].sort()};}
  ...
  function distByWorker(month){const m=new Map(); distMonth(month).forEach(d=>{const wc=codeNorm(d.worker_employee_code||d.employee_code||d.worker_code); if(!wc)return; if(!m.has(wc))m.set(wc,[]); m.get(wc).push(d);}); return m;}
  function supGroupsFromDist(month){const m=new Map(); distMonth(month).forEach(d=>{const sc=codeNorm(d.supervisor_employee_code||d.supervisor_code||d.supervisor_id||''); const sn=S(d.supervisor_name||sc||'بدون مشرف'); if(!m.has(sc))m.set(sc,{code:sc,name:sn,distRows:[],workers:new Set()}); const g=m.get(sc); g.distRows.push(d); const wc=codeNorm(d.worker_employee_code||d.employee_code||d.worker_code); if(wc)g.workers.add(wc);}); return m;}
  function displayProjects(rows){return [...new Set((rows||[]).map(d=>S(d.project_name||projectName(d.project_id)||d.project_id)).filter(Boolean))].join('، ')}

  function makeRow(emp,distRows,month,idx,groupTitle){
    const code=codeOf(emp), sv=savedFor({employee_ts_id:code},month), att=attendanceStats({employee_ts_id:code,employee_name:nameOf(emp),residency_name:resName(emp)},month);
    const job=sv.job_title||jobOf(emp); const cat=catOf(job);
    const projects=displayProjects(distRows); const supName=(distRows&&distRows[0])?S(distRows[0].supervisor_name):'';
    let start=sv.start_date||startOf(emp)||monthStart(month); let end=sv.end_date||endOf(emp)||monthEnd(month); start=clamp(start,month)||monthStart(month); end=clamp(end,month)||monthEnd(month);
    const basicDefault=cat==='supervisors'?2000:(cat==='guards'?1200:1300); const allowanceDefault=cat==='supervisors'?300:200;
    const row={entity_type:'unified',entity_id:code,employee_ts_id:sv.employee_ts_id||code,residency_name:sv.residency_name||resName(emp),employee_name:sv.employee_name||nameOf(emp),iqama_no:sv.iqama_no||iqamaOf(emp),work_location:sv.work_location||(cat==='supervisors'?'FM':(supName||projects||S(emp.work_location)||'غير موزع')),project_name:sv.project_name||projects,supervisor_name:cat==='supervisors'?(nameOf(emp)||supName):supName,job_title:job,start_date:start,end_date:end,work_days:daysBetween(start,end,month),absent_days:(sv.absent_days!==undefined&&S(sv.absent_days)!=='')?N(sv.absent_days):N(att.absent),basic_salary:(sv.basic_salary!==undefined&&S(sv.basic_salary)!=='')?N(sv.basic_salary):(basicOf(emp)||basicDefault),allowance:(sv.allowance!==undefined&&S(sv.allowance)!=='')?N(sv.allowance):(allowanceOf(emp)||allowanceDefault),commission:N(sv.commission??0),advance_deduction:N(sv.advance_deduction??0),manual_extra_deductions:N(sv.manual_extra_deductions??0),notes:sv.notes||'',row_order:idx,_groupTitle:groupTitle||''};
    return calc(row,month);
  }

  function buildRows(){
    const month=$('salaryMonth')?.value||today().slice(0,7), type=$('salaryType')?.value||'all', supFilter=codeNorm($('salarySupervisor')?.value), projectFilter=S($('salaryProject')?.value), search=norm($('salarySearch')?.value);
    const byCode=new Map(st.workers.map(w=>[codeOf(w),w])); const dByW=distByWorker(month), groups=supGroupsFromDist(month); const rows=[]; let n=0; const used=new Set();
    function allowedByType(emp){const c=catOf(jobOf(emp)); return type==='all'||type===c;}
    function allowedBySearch(r){return !search||[r.employee_ts_id,r.residency_name,r.employee_name,r.iqama_no,r.work_location,r.project_name,r.supervisor_name,r.job_title].join(' ').toLowerCase().includes(search);}

    groups.forEach(g=>{
      if(supFilter && codeNorm(g.code)!==supFilter) return;
      const gDist=projectFilter?g.distRows.filter(d=>S(d.project_id)===projectFilter):g.distRows;
      if(projectFilter && !gDist.length) return;
      const supEmp=byCode.get(codeNorm(g.code));
      const supTitle='المشرف: '+(supEmp?nameOf(supEmp):g.name)+' - '+codeNorm(g.code);
      if(supEmp && (type==='all'||type==='supervisors')){const sr=makeRow(supEmp,gDist,month,++n,supTitle); if(allowedBySearch(sr)){rows.push(Object.assign(sr,{_isSupervisor:true})); used.add(codeOf(supEmp));}}
      [...g.workers].forEach(wc=>{const emp=byCode.get(codeNorm(wc)); if(!emp||!allowedByType(emp))return; let wr=(dByW.get(codeNorm(wc))||[]).filter(d=>!projectFilter||S(d.project_id)===projectFilter); if(projectFilter&&!wr.length)return; const r=makeRow(emp,wr,month,++n,supTitle); if(allowedBySearch(r))rows.push(r); used.add(codeOf(emp));});
    });

    // موظفون لا يوجد لهم توزيع هذا الشهر: يظهرون فقط بدون فلتر مشروع/مشرف، حتى لا تضيع رواتب الفنيين والحراس والمشرفين غير الموزعين.
    if(!supFilter&&!projectFilter){
      const buckets=[['supervisors','مشرفون غير موزعين'],['technicians','الفنيون'],['guards','الحراس'],['workers','عمال غير موزعين']];
      buckets.forEach(([cat,title])=>{st.workers.forEach(emp=>{if(used.has(codeOf(emp)))return; if(catOf(jobOf(emp))!==cat)return; if(type!=='all'&&type!==cat)return; const r=makeRow(emp,[],month,++n,title); if(allowedBySearch(r))rows.push(r); used.add(codeOf(emp));});});
    }
    st.rows=rows; render(); msg('تم بناء الرواتب من العمال الموحدين + التوزيع + الحضور: '+rows.length+' سجل');
  }
  ...
  function input(r,k,w=110,type='text'){return `<input class="sal-input" style="width:${w}px" type="${type}" value="${esc(r[k]??'')}" oninput="tasneefSalariesUnifiedV438.update('${esc(r.entity_id)}','${k}',this.value)" onchange="tasneefSalariesUnifiedV438.update('${esc(r.entity_id)}','${k}',this.value)">`}
  ...
  function inject(){let sec=$('salaries'); if(!sec){const side=document.querySelector('.side'); const ref=[...document.querySelectorAll('.side .nav')].find(b=>S(b.textContent).includes('الأوقات الشهرية')); const btn=document.createElement('button'); btn.className='nav'; btn.textContent='الرواتب'; btn.onclick=function(){showPage('salaries',this); setTimeout(load,60)}; if(side)side.insertBefore(btn,ref?ref.nextSibling:side.querySelector('.nav.danger')); const main=document.querySelector('main.content'); sec=document.createElement('section'); sec.id='salaries'; sec.className='page hidden'; if(main)main.appendChild(sec);} sec.innerHTML=`<style>.salary-table-wrap{max-height:640px;overflow:auto}.salary-table{min-width:2300px}.salary-group-row td{background:#dfeee9;color:#064537;font-weight:900;text-align:right;font-size:13px}.salary-table th{position:sticky;top:0;z-index:2}.sal-input{width:110px;border:1px solid var(--line,#dce6e2);border-radius:8px;padding:6px;text-align:center}.salary-actions{display:flex;gap:8px;flex-wrap:wrap}.salary-note{background:#eef8f4;border:1px solid var(--line,#dce6e2);border-radius:14px;padding:10px;color:#07513f;font-weight:800}</style><div class="card"><div class="table-head"><h2>الرواتب</h2><span class="badge green">${VERSION}</span></div><div class="salary-note">التصحيح: مصدر كشف الرواتب هو العمال الموحدون أولاً، ثم التوزيع للمشرف/المشروع، ثم الحضور للغياب. لا يتم بناء الرواتب من التوزيع فقط.</div><div id="salaryMsg" class="msg hidden"></div><div class="filters"><div><label>الشهر</label><input type="month" id="salaryMonth" value="${today().slice(0,7)}" onchange="tasneefSalariesUnifiedV438.load()"></div><div><label>نوع الكشف</label><select id="salaryType" onchange="tasneefSalariesUnifiedV438.buildRows()"><option value="all">الكل</option><option value="supervisors">رواتب المشرفين</option><option value="workers">رواتب العمال</option><option value="technicians">رواتب الفنيين</option><option value="guards">رواتب الحراس</option></select></div><div><label>المشرف</label><select id="salarySupervisor" onchange="tasneefSalariesUnifiedV438.buildRows()"><option value="">كل المشرفين</option></select></div><div><label>المشروع</label><select id="salaryProject" onchange="tasneefSalariesUnifiedV438.buildRows()"><option value="">كل المشاريع</option></select></div><div><label>بحث</label><input id="salarySearch" oninput="tasneefSalariesUnifiedV438.buildRows()" placeholder="اسم/إقامة/TS"></div></div><div class="salary-actions"><button onclick="tasneefSalariesUnifiedV438.load()">تحديث الرواتب</button><button class="light" onclick="tasneefSalariesUnifiedV438.save(false)">حفظ التعديلات</button><button class="light" onclick="tasneefSalariesUnifiedV438.save(true)">اعتماد الرواتب</button><button class="light" onclick="tasneefSalariesUnifiedV438.print()">طباعة</button><button class="light" onclick="tasneefSalariesUnifiedV438.csv()">تصدير CSV</button></div><div id="salaryKpis" class="kpis small"></div><div class="table-wrap salary-table-wrap"><table class="salary-table"><thead><tr><th>رقم</th><th>أيدي الموظف</th><th>الشهر</th><th>اسم الموظف في الإقامة</th><th>اسم الموظف الحركي</th><th>رقم الإقامة</th><th>مكان العمل</th><th>ملاحظات</th><th>الوظيفة</th><th>بداية الخدمة</th><th>نهاية الخدمة</th><th>أيام العمل</th><th>أيام الغياب</th><th>الأيام المستحقة</th><th>قيمة الرواتب الأساسية</th><th>البدلات</th><th>الإجمالي</th><th>إجمالي الراتب على أيام الفترة</th><th>العمولات</th><th>الخصومات</th><th>جبر الكسور</th><th>خصم السلف</th><th>الصافي</th></tr></thead><tbody id="salaryBody"></tbody><tfoot id="salaryFoot"></tfoot></table></div></div>`;}

  const api={inject,load,buildRows,update,save,print,csv};
  window.tasneefSalariesUnifiedV438=api;
  // توافق مع تبويب الرواتب السابق حتى لا تختفي الصفحة
  window.tasneefSalariesUnifiedV435=api; window.tasneefSalariesV10267=api; window.tasneefSalariesV10281=api;
