(function(){
  'use strict';
  if(window.__tasneefCoreSalaryTabV437) return;
  window.__tasneefCoreSalaryTabV437=true;
  ...
  function ensureTab(){
    ...
    if(!btn.__salaryStableV437){
      btn.__salaryStableV437=true;
  ...
  function mountSalarySection(){
    ...
    if(!window.tasneefSalariesUnifiedV435){
      mount.innerHTML='<div class="cu413-msg err">ملف الرواتب لم يتم تحميله بعد. ارفع tasneef_salaries_unified_source_v435.js ثم حدّث الكاش.</div>';
      return false;
    }
    ...
    try{ window.tasneefSalariesUnifiedV435.inject(); }catch(e){ mount.innerHTML='<div class="cu413-msg err">تعذر إنشاء الرواتب: '+(e.message||e)+'</div>'; return false; }
    const sal=$('salaries');
  ...
  function showSalary(){
    ...
    try{
      ...
      if(mountSalarySection()){
        setTimeout(()=>{ try{ window.tasneefSalariesUnifiedV435?.load?.(); }catch(e){ msg('تعذر تحديث الرواتب: '+(e.message||e),true); } },60);
  ...
  window.tasneefCoreSalaryTabV437={show:showSalary,patch};
})();
