/* ===== V10196 ROOT REBUILD: Monthly section rebuilt from zero =====
   Source of truth: current active projects. Logs are used only for time calculation. */
(function(){
  'use strict';
  if(window.__tasneefMonthlyRebuildV10196) return;
  window.__tasneefMonthlyRebuildV10196 = true;
  ...
  function logDay(l){return S(l.log_date||l.date||l.work_date||l.check_in||l.created_at).slice(0,10)}
  function actualMinutes(l){
    try{ if(typeof window.logActualMinutes==='function') return N(window.logActualMinutes(l)); }catch(_){ }
    if(l.check_in&&l.check_out){try{ if(typeof window.minutesBetween==='function') return N(window.minutesBetween(l.check_in,l.check_out)); }catch(_){ }
      const a=new Date(l.check_in), b=new Date(l.check_out); const mins=(b-a)/60000; if(Number.isFinite(mins)&&mins>0) return mins;
    }
    return N(l.duration_minutes||l.actual_minutes||l.minutes||l.total_minutes||0);
  }
  ...
  async function loadFresh(){
    const r=monthRange();
    if(window.sb){
      const [projects,workers,users,logs1,logs2,assigns]=await Promise.all([
        ...
        safeSelect('time_logs', sb.from('time_logs').select('*').gte('log_date',r.from).lt('log_date',r.to)),
        safeSelect('time_logs', sb.from('time_logs').select('*').gte('check_in',r.fromIso).lt('check_in',r.toIso)),
        safeSelect('worker_project_assignments', sb.from('worker_project_assignments').select('*'))
      ]);
      const map=new Map(); [...logs1,...logs2].forEach((l,i)=>map.set(id(l.id)||('x'+i+'_'+Math.random()),l));
      state.projects=projects; state.workers=workers; state.users=users; state.logs=[...map.values()]; state.assignments=assigns;
      const d=D(); d.projects=projects; d.workers=workers; d.users=users; d.supervisors=users.filter(u=>u.role==='supervisor'&&u.is_active!==false); d.logs=state.logs; d.workerAssignments=assigns;
    }else{
      const d=D(); state.projects=d.projects||[]; state.workers=d.workers||[]; state.users=d.users||d.supervisors||[]; state.logs=d.logs||[]; state.assignments=d.workerAssignments||[];
    }
    state.loaded=true;
  }
  ...
  function buildMonthlyTimesFromCurrentProjects(){
    ...
    state.logs.forEach(l=>{
      ...
      const pid=id(l.project_id); if(!liveIds.has(pid)) return;
  ...
  async function render(force){
    ...
    window.__monthlyRowsRebuildV10196=rows;
    ...
    const sum=$('rbSummary'); if(sum) sum.innerHTML=`<div class="rb-kpi"><small>المشاريع الحالية</small><b>${rows.length}</b></div><div class="rb-kpi"><small>المشرفون</small><b>${sups}</b></div><div class="rb-kpi"><small>عدد السجلات</small><b>${logs}</b></div><div class="rb-kpi"><small>إجمالي الدقائق</small><b>${Math.round(total).toLocaleString('en-US')}</b></div><div class="rb-kpi"><small>وقت الانتقال</small><b>${Math.round(travel).toLocaleString('en-US')}</b></div>`;
  ...
  window.monthlyRowsV60=()=>window.__monthlyRowsRebuildV10196||buildMonthlyTimesFromCurrentProjects();
  ...
  console.log('Tasneef V10196 monthly rebuilt from zero loaded');
})();
