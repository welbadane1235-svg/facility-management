...
(function(){
  'use strict';
  const VERSION='403'; // approved hotfix: fresh projects type from server on Show
  ...
  async function loadDynamicMonth(month, force){
    if(!force && currentMonthLoaded===month && DYNAMIC_DATA.length) return DYNAMIC_DATA;
  ...
  async function getRows(force){const m=selectedMonth(); if(m==='2026-06') return normalize(await loadJune(),m); return await loadDynamicMonth(m, !!force);}
  ...
  function bind(){
    const show=$('mc401Show'); if(show&&show.dataset.v403!=='1'){show.dataset.v403='1'; show.addEventListener('click',()=>render(true));}
    const mon=$('mc401Month'); if(mon&&mon.dataset.v403!=='1'){mon.dataset.v403='1'; mon.addEventListener('change',()=>render(true));}
    ['mc401Supervisor','mc401Type'].forEach(id=>{const el=$(id); if(!el||el.dataset.v403==='1')return; el.dataset.v403='1'; el.addEventListener('change',()=>render(false));});
    const p=$('mc401Print'); if(p&&p.dataset.v403!=='1'){p.dataset.v403='1'; p.addEventListener('click',printReport);}
    const e=$('mc401Export'); if(e&&e.dataset.v403!=='1'){e.dataset.v403='1'; e.addEventListener('click',csv);}
  }
  document.addEventListener('DOMContentLoaded',()=>{bind(); loadJune().then(()=>render(true));}); setTimeout(()=>{bind(); render(true);},1000);
})();
