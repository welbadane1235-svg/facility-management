...
(function(){
  'use strict';
  if(window.__tasneefInventoryAuditRootV10054) return;
  window.__tasneefInventoryAuditRootV10054 = true;
  ...
  function removeOldVersionMark(){
    try{
      const badText = /Tasneef\s+v\d+|worker-multi-project|automation-export/i;
      document.querySelectorAll('body *').forEach(el => {
        const txt = S(el.textContent);
        const cs = getComputedStyle(el);
        const fixedLike = cs.position === 'fixed' || cs.position === 'sticky';
        const nearLeftBottom = (parseInt(cs.left||'999',10) <= 30 || parseInt(cs.right||'999',10) <= 30) && parseInt(cs.bottom||'999',10) <= 30;
        if(fixedLike && nearLeftBottom && txt && badText.test(txt) && !el.closest('.side') && !el.closest('#inventoryAudit')){
          el.remove();
        }
      });
      document.querySelectorAll('a[title],button[title]').forEach(el=>{
        if(badText.test(S(el.getAttribute('title')))) el.removeAttribute('title');
      });
    }catch(_){}
  }
  ...
  function ensureStyle(){
    if($('auditStyleV10054')) return;
    const st = document.createElement('style');
    st.id = 'auditStyleV10054';
    st.textContent = `
      .inventory-audit-page-v10054 .audit-toolbar-v10054{display:flex;gap:8px;flex-wrap:wrap;align-items:end}
      .audit-kpis-v10054{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin:12px 0}
      .audit-kpi-v10054{background:#fff;border:1px solid var(--line,#dce6e2);border-radius:16px;padding:12px}
      .audit-kpi-v10054 small{color:#60706a}.audit-kpi-v10054 b{display:block;color:#0A4033;font-size:22px;margin-top:5px}
      .audit-open-banner-v10054{background:#eef8f4;border:1px solid #cfe4dc;color:#064b38;border-radius:14px;padding:10px 12px;margin:10px 0;font-weight:800}
      .audit-table-v10054 input,.audit-table-v10054 select{min-width:90px}
      .audit-glass-overlay-v10054{position:absolute;inset:0;z-index:999998;background:rgba(245,250,248,.72);backdrop-filter:blur(7px);-webkit-backdrop-filter:blur(7px);display:flex;align-items:center;justify-content:center;padding:18px;border-radius:22px}
      .audit-glass-card-v10054{width:min(560px,92vw);background:rgba(255,255,255,.86);border:1px solid rgba(207,226,220,.95);box-shadow:0 22px 60px rgba(0,45,35,.16);border-radius:26px;padding:26px;text-align:center;color:#063f32}
      .audit-glass-card-v10054 h2{margin:0 0 10px;color:#063f32}.audit-glass-card-v10054 p{line-height:1.9;color:#35554c}
      .audit-glass-actions-v10054{display:flex;gap:10px;justify-content:center;flex-wrap:wrap;margin-top:14px}
      @media(max-width:900px){.audit-kpis-v10054{grid-template-columns:1fr 1fr}}
      @media print{body *{visibility:hidden!important}#inventoryAudit,#inventoryAudit *{visibility:visible!important}#inventoryAudit{position:absolute;inset:0;background:#fff}.side,.hero,.audit-no-print-v10054{display:none!important}}
    `;
  ...
  function sectionHtml(){
    return `
      <div class="card section-head audit-head-v10054">
        <div>
          <h2>قسم الجرد</h2>
          <p>جرد مستقل مرتبط بالمالية والمخزون. عند فتح عملية جرد يتم قفل المالية والمخزون حتى إغلاق العملية.</p>
        </div>
        <div class="actions audit-no-print-v10054">
          <button onclick="tasneefInventoryAuditV10054.newAudit()">+ عملية جرد جديدة</button>
          <button class="light" onclick="tasneefInventoryAuditV10054.load(true)">تحديث</button>
        </div>
      </div>

      <div id="auditLockMsgV10054" class="audit-open-banner-v10054 hidden"></div>

      <div class="card">
        <div class="table-head">
          <h2>عمليات الجرد</h2>
          <div class="row-actions audit-no-print-v10054">
            <button class="light" onclick="tasneefInventoryAuditV10054.load(true)">تحديث</button>
            <button class="light" onclick="tasneefInventoryAuditV10054.print()">طباعة</button>
            <button class="light" onclick="tasneefInventoryAuditV10054.exportCsv()">تصدير CSV</button>
            <button class="light" onclick="tasneefInventoryAuditV10054.reloadOpenItems(true)">إعادة تحميل بنود الجرد</button>
          </div>
        </div>
        <div id="auditSessionsV10054" class="quick-list"></div>
      </div>

      <div class="card" id="auditDetailsCardV10054">
        <div class="table-head">
          <h2 id="auditTitleV10054">تفاصيل الجرد</h2>
          <div class="row-actions audit-no-print-v10054">
            <button class="light" onclick="tasneefInventoryAuditV10054.reloadOpenItems(true)">تحميل المنتجات</button>
            <button class="light" onclick="tasneefInventoryAuditV10054.closeAudit(false)">إغلاق بدون تعديل المخزون</button>
          </div>
        </div>
        <div class="filters audit-no-print-v10054">
          <input id="auditSearchV10054" placeholder="بحث باسم المنتج أو الكود" oninput="tasneefInventoryAuditV10054.renderCurrentItems()">
          <select id="auditStatusFilterV10054" onchange="tasneefInventoryAuditV10054.renderCurrentItems()">
            <option value="">كل الحالات</option>
            <option>لم يتم جرده</option><option>مطابق</option><option>نقص</option><option>زيادة</option>
          </select>
        </div>
        <div id="auditSummaryV10054" class="audit-kpis-v10054"></div>
        <div class="table-wrap">
          <table class="audit-table-v10054">
            ...
            <tbody id="auditItemsBodyV10054"></tbody>
          </table>
        </div>
      </div>`;
  }
  ...
  function ensureSection(){
    ...
    if(!sec){
      ...
      sec.className = 'page hidden inventory-audit-page-v10054';
      main.appendChild(sec);
    }
    if(sec.dataset.auditV10054 !== '1'){
      sec.classList.add('inventory-audit-page-v10054');
      sec.innerHTML = sectionHtml();
      sec.dataset.auditV10054 = '1';
    }
  }
  ...
  async function renderSessions(){
    const wrap = $('auditSessionsV10054') || $('auditSessionsV10046');
    ...
    wrap.innerHTML = rows.map(a => `
      <div class="quick-item">
        ...
        <button class="light" onclick="tasneefInventoryAuditV10054.renderItems(${Number(a.id)})">عرض</button>
  ...
  function summaryHtml(rows){
    ...
    return `
      <div class="audit-kpi-v10054"><small>إجمالي المنتجات</small><b>${s.total}</b></div>
      <div class="audit-kpi-v10054"><small>تم جردها</small><b>${s.done}</b></div>
      <div class="audit-kpi-v10054"><small>مطابق</small><b>${s.equal}</b></div>
      <div class="audit-kpi-v10054"><small>نقص / زيادة</small><b>${s.shortage} / ${s.extra}</b></div>`;
  }
  ...
  async function renderItems(auditId, auditObj=null){
    ...
    const body = $('auditItemsBodyV10054') || $('auditItemsBodyV10046');
    ...
    const search = S($('auditSearchV10054')?.value).toLowerCase();
    const fstatus = S($('auditStatusFilterV10054')?.value);
    ...
    if($('auditTitleV10054')) $('auditTitleV10054').textContent = `تفاصيل الجرد ${E(currentAudit?.audit_no || auditId)}`;
    if($('auditSummaryV10054')) $('auditSummaryV10054').innerHTML = summaryHtml(rows);
  ...
  function rowHtml(it){
    ...
    return `<tr data-audit-item-id="${Number(it.id)}">
      ...
      <td><input type="number" step="0.01" value="${has ? E(it.actual_qty) : ''}" onchange="tasneefInventoryAuditV10054.saveQty(${Number(it.id)},this.value,${N(it.system_qty)})"></td>
      <td>${diff === null ? '-' : diff.toFixed(2)}</td>
      <td><span class="badge ${cls}">${st}</span></td>
      <td><input value="${E(it.notes || '')}" onchange="tasneefInventoryAuditV10054.saveNote(${Number(it.id)},this.value)"></td>
    </tr>`;
  }
  ...
  async function load(force=false){
    ...
    const box = $('auditLockMsgV10054') || $('auditLockMsgV10046');
    if(box){
    ...
    if(currentAudit){
      ...
      if($('auditSummaryV10054')) $('auditSummaryV10054').innerHTML = '';
    }
  ...
  function financeElements(fin){
    return [...fin.querySelectorAll('input,select,textarea,button')].filter(el => !el.closest('#financeAuditGlassV10054'));
  }

  function restoreFinance(fin){
    const lock = $('financeAuditGlassV10054');
    lock?.remove();
    financeElements(fin).forEach(el => {
      if(el.dataset.auditDisabledV10054 === '1'){
        if(el.dataset.auditWasDisabledV10054 !== '1') el.disabled = false;
        delete el.dataset.auditDisabledV10054;
        delete el.dataset.auditWasDisabledV10054;
      }
    });
  }
  ...
  async function applyFinanceLock(){
    ...
    financeElements(fin).forEach(el => {
      if(el.dataset.auditDisabledV10054 !== '1'){
        el.dataset.auditWasDisabledV10054 = el.disabled ? '1' : '0';
        el.dataset.auditDisabledV10054 = '1';
      }
      el.disabled = true;
    });
    let overlay = $('financeAuditGlassV10054');
    if(!overlay){
      overlay = document.createElement('div');
      overlay.id = 'financeAuditGlassV10054';
      overlay.className = 'audit-glass-overlay-v10054';
      fin.appendChild(overlay);
    }
    overlay.innerHTML = `<div class="audit-glass-card-v10054">
      ...
      <div class="audit-glass-actions-v10054">
        <button onclick="showPage('inventoryAudit',document.querySelector('[data-page=inventoryAudit]')); setTimeout(function(){tasneefInventoryAuditV10054.load(true)},80)">فتح الجرد</button>
        <button class="light" onclick="tasneefInventoryAuditV10054.load(true)">تحديث</button>
      </div>
    </div>`;
  }
  ...
  window.tasneefInventoryAuditV10054 = api;
