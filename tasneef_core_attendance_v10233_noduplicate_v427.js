(function(){
  'use strict';
  const BUILD='V427 نفس بيانات الحضور القديم - بدون تكرار';
  ...
  function attWorkerKey(a){
    const code=S(a.worker_employee_code||a.worker_code||a.employee_code||a.worker_id||'');
    if(code)return 'code:'+norm(code);
    const name=S(a.worker_identity||a.worker_name||a.name||'');
    return name ? 'name:'+norm(name) : '';
  }
  function buildAttMap(){
    const m=new Map();
    (state.att||[]).forEach(a=>{
      const d=dateKey(a.attendance_date||a.date||a.created_at); const wk=attWorkerKey(a); if(!d||!wk)return;
      const k=wk+'|'+d; const old=m.get(k);
  ...
  function rowAttKey(r,date){return workerUniqKey(r)+'|'+date;}
  ...
  async function load(month,force){
    ...
    const dr=await safe('monthly_distribution', c.from('monthly_distribution').select('*').eq('month_key',month).limit(20000));
    const ar=await safe('attendance', c.from('attendance').select('*').gte('attendance_date',month+'-01').lt('attendance_date',nextMonth(month)+'-01').limit(20000));
    state={dist:mergeDistribution(dr.data||[]),att:ar.data||[],month};
  }
  ...
  function render(){
    ...
    let p=0,a=0,o=0; rows.forEach(r=>{const st=statusCode(amap.get(rowAttKey(r,f.date))?.status||'present'); if(st==='present')p++; else if(st==='absent')a++; else o++;});
    ...
    $('cu427Workers').innerHTML=[...groups.values()].map(g=>`<div class="cu427-worker cu427-super"><span>المشرف: ${esc(g.name)}</span><span>عدد العمال: ${g.items.length}</span></div>`+g.items.map(r=>{
      const rec=amap.get(rowAttKey(r,f.date)); const st=statusCode(rec?.status||'present'); const projects=(r.__projects&&r.__projects.length?r.__projects.join('، '):distProject(r));
  ...
  async function save(){
    try{const c=client(); if(!c)return msg('Supabase غير جاهز',true); const f=filters(); const rows=currentRows(); if(!rows.length)return msg('لا يوجد عمال للحفظ.',true);
      ...
      const payload=rows.map(r=>{const el=card.get(workerUniqKey(r)); if(!el)return null; return {worker_identity:distName(r),worker_name:distName(r),worker_employee_code:distCode(r),worker_code:distCode(r),project_id:distProjectId(r)||((r.__projectIds||[])[0]||''),supervisor_id:distSupCode(r),supervisor_employee_code:distSupCode(r),status:S(el.querySelector('select')?.value||'present'),notes:S(el.querySelector('input')?.value||'')};}).filter(Boolean);
      let saved=0; const rpc=await safe('attendance rpc', c.rpc('tasneef_save_attendance_by_name_v10215',{p_date:f.date,p_records:payload}));
      if(!rpc.error){saved=N(rpc.data?.saved)||payload.length;}
      else{
        for(const x of payload){
          const found=await safe('attendance find',c.from('attendance').select('id').eq('attendance_date',f.date).or(`worker_identity.eq.${x.worker_identity},worker_name.eq.${x.worker_identity},worker_employee_code.eq.${x.worker_employee_code}`).limit(1));
          const row={attendance_date:f.date,month_key:f.date.slice(0,7),worker_identity:x.worker_identity,worker_name:x.worker_name,worker_employee_code:x.worker_employee_code,project_id:x.project_id,supervisor_id:x.supervisor_id,supervisor_employee_code:x.supervisor_employee_code,status:x.status,notes:x.notes,check_time:new Date().toISOString(),source:'v427_old_style_no_duplicate'};
          let res; if((found.data||[])[0]?.id)res=await c.from('attendance').update(row).eq('id',found.data[0].id); else res=await c.from('attendance').insert(row); if(res?.error)throw res.error; saved++;
        }
      }
  ...
  window.tasneefCoreAttendanceV427={refresh,save,activate};
  console.log('Tasneef core attendance V427 loaded');
})();
