(function(){
  'use strict';
  if(window.__tasneefCoreSalaryTabV439) return;
  window.__tasneefCoreSalaryTabV439=true;
  ...
  function ensureTab(){
    ...
    if(!btn.__salaryStableV439){
      btn.__salaryStableV439=true;
  ...
  function mountSalarySection(){
    ...
    if(!window.tasneefSalariesUnifiedV439){
      mount.innerHTML='<div class="cu413-msg err">ملف الرواتب لم يتم تحميله بعد. ارفع tasneef_salaries_unified_source_v439.js ثم حدّث الكاش.</div>';
      return false;
    }
    ...
    try{ window.tasneefSalariesUnifiedV439.inject(); }catch(e){ mount.innerHTML='<div class="cu413-msg err">تعذر إنشاء الرواتب: '+(e.message||e)+'</div>'; return false; }
    const sal=$('salaries');
  ...
  function showSalary(){
    ...
    try{
      ...
      if(mountSalarySection()){
        setTimeout(()=>{ try{ window.tasneefSalariesUnifiedV439?.load?.(); }catch(e){ msg('تعذر تحديث الرواتب: '+(e.message||e),true); } },60);
  ...
  window.tasneefCoreSalaryTabV439={show:showSalary,patch};
})();
